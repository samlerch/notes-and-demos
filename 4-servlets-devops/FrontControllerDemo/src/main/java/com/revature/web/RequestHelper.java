...
public class RequestHelper {
	...
	public static void processLogin(HttpServletRequest request, HttpServletResponse response)
		...
		// IF the user is not null, save the user to the session, and print the user's
		// info to the client using print writer
		if (e != null) {

			// grab the sessiona dn add the user to it
			HttpSession session = request.getSession();
			session.setAttribute("the-user", e); // binding the retrieved user object to the session and giving it the
													// key "the-user"
	...
	public static void processEmployees(HttpServletRequest request, HttpServletResponse response)
		...
		List<Employee> allEmployees = eserv.findAll(); // this calls our DAO layer, which retrieves all objects from the
														// DB

		// 3. Marshall the list of Java Objects to JSON (using Jackson as our Object
		// Mapper)
		String jsonString = om.writeValueAsString(allEmployees);

		// 4. Call the Print Writer to write it out to the client (browser) in the
		// response body
		PrintWriter out = response.getWriter();
	...
	public static void processError(HttpServletRequest request, HttpServletResponse response)
		...
		/**
		 * forward() differs from sendRedirect() in that it DOES NOT produce a new
		 * request, but rather just forwards the same request to a new
		 * resource(maintaining the URL).
		 */
