...
public class SessionServlet extends HttpServlet {
	...
	/**
	 * When we send a GET request this servlet, (at /sesserv url) we will a Villain
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// Use the print writer from the response object to print out some message to
		// the
		// browser about what happened
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		/**
		 * The purpose of this method is to capture the parameters from the HTTP Request
		 * that is sent to the /sesserv URL, then instnatiate an object from them and
		...
		session.setAttribute("the-villain", vill); // in our HelperSessionServlet we are capturing the object with the
													// "the-villain"
		// key from the session, so we'll set this villain object with the same session

		// 5. Print to the screen (using Print Writer) that the villain object was
		// successfully sent to the session
		PrintWriter out = response.getWriter();
		out.write(new ObjectMapper().writeValueAsString(vill));
		out.println("A villain is on the loose...(in the session)");

		// After it's been set to the session, you can retrieve it by sending a GET
		// request to the HelperSessionServlet
